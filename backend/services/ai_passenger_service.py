...
def get_ai_runtime_config() -> dict[str, str]:
    ...
    if not inference_script.exists():
        raise FileNotFoundError(
            f"AI inference script is missing at {inference_script}"
        )
...
def get_last_ai_count(bus_id: str, live_status_data: dict[str, Any] | None = None) -> int:
    return get_last_ai_metrics(bus_id, live_status_data)["estimated_passenger_count"]
...
def launch_ai_counting(
    *,
    ...
    with AI_SESSION_LOCK:
        ...
        if existing_session:
            raise RuntimeError(f"An AI session is already running for bus {bus_id}")
    ...
    print(f"[FLOW] AI subprocess launched for bus {bus_id} with PID {process.pid}.")
...
def stop_ai_counting(
    ...
    if not session:
        metrics = get_last_ai_metrics(bus_id)
        _update_live_status(
            bus_id,
            {
                "ai_state": "stopped",
        ...
        print(
            f"[FLOW] End Session received for {bus_id}, but no active AI subprocess was found."
        )
        return {
            "was_running": False,
    ...
    process: subprocess.Popen[str] = session["process"]
    print(f"[FLOW] End Session received for {bus_id}.")
    _update_live_status(
        bus_id,
        {
            "ai_state": "stopping",
    ...
    if process.poll() is None:
        try:
            ...
            print(
                f"[FLOW] Graceful stop timed out for {bus_id}. Terminating AI subprocess."
            )
    ...
    if process.poll() is None:
        try:
            ...
            print(f"[FLOW] Force-killing AI subprocess for {bus_id}.")
            process.kill()
            try:
                process.wait(timeout=2)
            except subprocess.TimeoutExpired:
    ...
    print(f"[FLOW] AI subprocess stopped for bus {bus_id}.")
    return {
        "was_running": True,
